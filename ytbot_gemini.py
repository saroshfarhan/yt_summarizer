...
def define_parameters():
    ...
    return {
        # Set the temperature for creativity vs determinism (0.0-1.0)
        "temperature": 0.7,
...
def initialize_watsonx_llm(model_id, api_key, parameters):
    ...
    return ChatGoogleGenerativeAI(
        ...
        google_api_key=api_key,      # Provide the Google API key
        temperature=parameters.get("temperature", 0.7),  # Set temperature for response creativity
        max_output_tokens=parameters.get("max_output_tokens", 900)  # Set max tokens for response length
    )

def setup_embedding_model(api_key):
    # Create and return an instance of GoogleGenerativeAIEmbeddings with the specified configuration
    return GoogleGenerativeAIEmbeddings(
        model="models/embedding-001",      # Set the model ID for Google's embedding model
        google_api_key=api_key              # Provide the Google API key
    )
...
def create_qa_chain(llm, prompt_template, verbose=True):
    ...
    Args:
        llm: Language model instance
            The language model to use in the chain (e.g., ChatGoogleGenerativeAI).
        prompt_template: PromptTemplate
...
def summarize_video(video_url):
    ...
    if processed_transcript:
        ...
        llm = initialize_watsonx_llm(model_id, api_key, define_parameters())
